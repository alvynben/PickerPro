...
if BATCHING:
    ...
    ROUTES_FILE = "batchRoutes.txt"
    LOCS_FILE = "batchLocs.txt"
else:
    IMAGE_NAME = "picker.png"
    ROUTES_FILE = "singleRoutes.txt"
    LOCS_FILE = "singleLocs.txt"
...
f = open(f"{CURRENT_DIRECTORY}/{LOCS_FILE}","r")
...
f = open(f"{CURRENT_DIRECTORY}/{ROUTES_FILE}","r")
lines = f.read().splitlines()
batch_routes = []
for line in lines:
    lineSplit = line.split(",")
